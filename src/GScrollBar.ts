...
export class GScrollBar extends GComponent {
  ...
  private $dragOffset: createjs.Point
  ...
  public set displayPerc(val: number) {
    if (this.$vertical) {
      if (!this.$fixedGripSize) this.$grip.height = val * this.$bar.height
      this.$grip.y = this.$bar.y + (this.$bar.height - this.$grip.height) * this.$scrollPerc
    } else {
      if (!this.$fixedGripSize) this.$grip.width = val * this.$bar.width
  ...
  public set scrollPerc(val: number) {
    ...
    if (this.$vertical)
      this.$grip.y = this.$bar.y + (this.$bar.height - this.$grip.height) * this.$scrollPerc
    else this.$grip.x = this.$bar.x + (this.$bar.width - this.$grip.width) * this.$scrollPerc
  }

  public get minSize(): number {
    if (this.$vertical)
      return (
        (this.$arrowButton1 != null ? this.$arrowButton1.height : 0) +
        (this.$arrowButton2 != null ? this.$arrowButton2.height : 0)
      )
    else
      return (
        (this.$arrowButton1 != null ? this.$arrowButton1.width : 0) +
  ...
  protected constructFromXML(xml: XmlNode): void {
    ...
    if (xml != null) this.$fixedGripSize = xml.attributes.fixedGripSize == 'true'
  ...
  private $gripMouseDown(evt: any): void {
    ...
    this.$mouseUpEvent = GRoot.inst.nativeStage.on(
      InteractiveEvents.Up,
      this.$gripDraggingEnd,
      this
    )
  }

  private static sScrollbarHelperPoint: createjs.Point = new createjs.Point()
  private $gripDragging(evt: any): void {
    let pt: createjs.Point = evt.target.localToLocal(evt.localX, evt.localY, this.$displayObject); // todo
  ...
  private $gripDraggingEnd(evt: createjs.Event): void {
  ...
  private $arrowButton1Click(evt: createjs.Event): void {
    evt.stopPropagation()

    if (this.$vertical) this.$target.scrollUp()
    else this.$target.scrollLeft()
  }

  private $arrowButton2Click(evt: createjs.Event): void {
    evt.stopPropagation()

    if (this.$vertical) this.$target.scrollDown()
    else this.$target.scrollRight()
  }

  private $barMouseDown(evt: any): void {
    let pt: createjs.Point = new createjs.Point(evt.localX, evt.localY)
    // let pt: createjs.Point = evt.data.getLocalPosition(this.$grip.displayObject, GScrollBar.sScrollbarHelperPoint);
    if (this.$vertical) {
      if (pt.y < 0) this.$target.scrollUp(4)
      else this.$target.scrollDown(4)
    } else {
      if (pt.x < 0) this.$target.scrollLeft(4)
      else this.$target.scrollRight(4)
    }
  }
  ...
  public dispose(): void {
    ...
    GScrollBar.sScrollbarHelperPoint = new createjs.Point();
    if (this.$arrowButton1) this.$arrowButton1.off(InteractiveEvents.Down, this.$arrowButton1Click)
    if (this.$arrowButton2) this.$arrowButton2.off(InteractiveEvents.Down, this.$arrowButton2Click)
