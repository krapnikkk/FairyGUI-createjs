...
import {
  ...
  GearType,
  Keys,
  ListChildrenRenderOrder,
  ListLayoutType,
  ListSelectionMode,
  OverflowType,
  ParseAlignType,
  ParseListChildrenRenderOrder,
  ParseListLayoutType,
...
export class GList extends GComponent {
  ...
  private $itemSize: createjs.Point
  ...
  public set apexIndex(value: number) {
    if (this.$apexIndex != value) {
      ...
      if (this.$childrenRenderOrder == ListChildrenRenderOrder.Arch) this.appendChildrenList()
    }
  }
  ...
  protected $setChildIndex(child: GObject, oldIndex: number, index: number = 0): number {
    ...
    if (child.inContainer) {
      ...
      if (this.$childrenRenderOrder == ListChildrenRenderOrder.Ascent) {
        ...
        if (displayIndex == this.$container.children.length) displayIndex--
        ...
        if (displayIndex == this.$container.children.length) displayIndex--
        this.$container.setChildIndex(child.displayObject, displayIndex)
      } else GTimer.inst.callLater(this.appendChildrenList, this)
  ...
  public dispose(): void {
    ...
    this.$pool.clear()
    if (this.$scrollPane) this.$scrollPane.off(ScrollEvent.SCROLL, this.$scrolled)
    super.dispose()
  }
  ...
  public set layout(value: ListLayoutType) {
    if (this.$layout != value) {
      this.$layout = value
      this.setBoundsChangedFlag()
      if (this.$virtual) this.setVirtualListChangedFlag(true)
    }
  }
  ...
  public set lineCount(value: number) {
    if (this.$lineCount != value) {
      ...
      if (
        this.$layout == ListLayoutType.FlowVertical ||
        this.$layout == ListLayoutType.Pagination
      ) {
        this.setBoundsChangedFlag()
        if (this.$virtual) this.setVirtualListChangedFlag(true)
      }
  ...
  public set columnCount(value: number) {
    if (this.$columnCount != value) {
      ...
      if (
        this.$layout == ListLayoutType.FlowHorizontal ||
        this.$layout == ListLayoutType.Pagination
      ) {
        this.setBoundsChangedFlag()
        if (this.$virtual) this.setVirtualListChangedFlag(true)
  ...
  public get virtualItemSize(): createjs.Point {
    return this.$itemSize
  }

  public set virtualItemSize(value: createjs.Point) {
    if (this.$virtual) {
      if (this.$itemSize == null) this.$itemSize = new createjs.Point()
  ...
  public get itemPool(): GObjectRecycler {
    return this.$pool
  }

  public getFromPool(url: string = null): GObject {
    if (!url) url = this.$defaultItem
  ...
  public removeChildrenToPool(beginIndex: number = 0, endIndex: number = -1): void {
    if (endIndex < 0 || endIndex >= this.$children.length) endIndex = this.$children.length - 1
    for (let i: number = beginIndex; i <= endIndex; ++i) this.removeChildToPoolAt(beginIndex)
  }
    ...
    if (this.$virtual) {
      for (i = 0; i < this.$realNumItems; i++) {
        ...
        if ((ii.obj instanceof GButton && ii.obj.selected) || (ii.obj == null && ii.selected)) {
          if (this.$loop) return i % this.$numItems
          else return i
        }
      }
    } else {
  ...
  public set selectedIndex(value: number) {
    if (value >= 0 && value < this.numItems) {
      if (this.selectionMode != ListSelectionMode.Single) this.clearSelection()
  ...
  public getSelection(): number[] {
    ...
    if (this.$virtual) {
      ...
      for (i = 0; i < cnt; i++) {
        const obj: GButton = this.$children[i] as GButton
        if (obj != null && obj.selected) ret.push(i)
      }
    }
    return ret
  }
  ...
  public addSelection(index: number, scrollIntoView: boolean = false): void {
    if (this.$selectionMode == ListSelectionMode.None) return

    this.checkVirtualList()

    if (this.$selectionMode == ListSelectionMode.Single) this.clearSelection()

    if (scrollIntoView) this.scrollToView(index)
    ...
    if (this.$virtual) {
      ...
      if (ii.obj != null) obj = ii.obj as GButton
      ii.selected = true
    } else obj = this.getChildAt(index) as GButton
  ...
  public handleArrowKey(key: Keys): void {
    ...
    switch (key) {
      ...
      case Keys.Right:
        if (
          this.$layout == ListLayoutType.SingleRow ||
          this.$layout == ListLayoutType.FlowHorizontal ||
          this.$layout == ListLayoutType.Pagination
        ) {
          index++
  ...
  private $clickItem(evt: createjs.Event): void {
    if (this.$scrollPane != null && this.$scrollPane.isDragging) return

    const item: GObject = GObject.castFromNativeObject(evt.currentTarget)
    if (!item) return
    this.setSelectionOnEvent(item)

    if (this.$scrollPane && this.scrollItemToViewOnClick) this.$scrollPane.scrollToView(item, true)
    let event = new createjs.Event(ListEvent.ItemClick, true, false)
    event.data = {item};
    this.dispatchEvent(event, this)
  }

  private setSelectionOnEvent(button: GObject): void {
    if (!(button instanceof GButton) || this.$selectionMode == ListSelectionMode.None) return
    ...
    if (this.$selectionMode == ListSelectionMode.Single) {
      ...
      if (DOMEventManager.inst.isKeyPressed(Keys.Shift)) {
        if (!button.selected) {
        ...
        DOMEventManager.inst.isKeyPressed(Keys.Ctrl) ||
    ...
    if (!dontChangeLastIndex) this.$lastSelectedIndex = index
  ...
  public handleControllerChanged(c: Controller): void {
    ...
    if (this.$selectionController == c) this.selectedIndex = c.selectedIndex
  }
  ...
  private updateSelectionController(index: number): void {
    if (
      ...
      const c: Controller = this.$selectionController
      this.$selectionController = null
  ...
  public getSnappingPosition(
    xValue: number,
    yValue: number,
    resultPoint: createjs.Point = null
  ): createjs.Point {
    if (this.$virtual) {
      ...
      if (
        ...
        if (
          index < this.$virtualItems.length &&
      ...
      return resultPoint
    } else return super.getSnappingPosition(xValue, yValue, resultPoint)
  }
  ...
  public scrollToView(index: number, ani: boolean = false, snapToFirst: boolean = false): void {
    if (this.$virtual) {
      ...
      if (index >= this.$virtualItems.length)
        throw new Error(
          `Invalid child index: ${index} is larger than max length: ${this.$virtualItems.length}`
        )

      if (this.$loop) index = Math.floor(this.$firstIndex / this.$numItems) * this.$numItems + index
      ...
      if (
        ...
        rect = new createjs.Rectangle(
          page * this.viewWidth + (index % this.$curLineItemCount) * (ii.width + this.$columnGap),
          ((index / this.$curLineItemCount) % this.$curLineItemCount2) *
          (ii.height + this.$lineGap),
          ii.width,
          ii.height
        )
      }
      ...
      snapToFirst = true
      if (this.$scrollPane != null) this.$scrollPane.scrollToView(rect, ani, snapToFirst)
    } else {
      const obj: GObject = this.getChildAt(index)
      if (this.$scrollPane != null) this.$scrollPane.scrollToView(obj, ani, snapToFirst)
  ...
  public childIndexToItemIndex(index: number): number {
    ...
    if (this.$layout == ListLayoutType.Pagination) {
      ...
      if (this.$loop && this.$numItems > 0) index = index % this.$numItems
      return index
    }
  }
  ...
  public itemIndexToChildIndex(index: number): number {
    ...
    if (this.$layout == ListLayoutType.Pagination)
      ...
      if (this.$loop && this.$numItems > 0) {
        ...
        if (index >= j) index = this.$firstIndex + (index - j)
        else index = this.$firstIndex + this.$numItems + (j - index)
      } else index -= this.$firstIndex
      return index
    }
  }
  ...
  private $setVirtual(loop: boolean): void {
    if (!this.$virtual) {
      if (this.$scrollPane == null) throw new Error('Virtual list must be scrollable')
      ...
      if (loop) {
        if (
          ...
          throw new Error(
            'Virtual list with loop mode is not supported for both FlowHorizontal and FlowVertical layout'
          )
      ...
      if (this.$itemSize == null) {
        ...
        if (obj == null)
          throw new Error(
            'Virtual list must have a default list item resource specified through list.defaultItem = resUrl.'
          )
        else {
      ...
      if (
        ...
        if (this.$loop) this.$scrollPane.$loop = 2
      } else {
        this.$scrollPane.scrollSpeed = this.$itemSize.x
        if (this.$loop) this.$scrollPane.$loop = 1
      }
  ...
  public set numItems(value: number) {
    ...
    if (this.$virtual) {
      if (this.itemRenderer == null) throw new Error('list.itemRenderer is required')

      this.$numItems = value
      if (this.$loop) this.$realNumItems = this.$numItems * 6
      //enlarge for loop
      else this.$realNumItems = this.$numItems
      ...
      if (this.$realNumItems > oldCount) {
        ...
        for (i = this.$realNumItems; i < oldCount; i++) this.$virtualItems[i].selected = false
      }
  ...
  private setVirtualListChangedFlag(layoutChanged: boolean = false): void {
    if (layoutChanged) this.$virtualListChanged = VirtualListChangedType.SizeChanged
  ...
  private $refreshVirtualList(): void {
    ...
    if (layoutChanged) {
      if (this.$layout == ListLayoutType.SingleColumn || this.$layout == ListLayoutType.SingleRow)
        ...
        if (this.$columnCount > 0) this.$curLineItemCount = this.$columnCount
        else {
          this.$curLineItemCount = Math.floor(
            (this.$scrollPane.viewWidth + this.$columnGap) / (this.$itemSize.x + this.$columnGap)
          )
          if (this.$curLineItemCount <= 0) this.$curLineItemCount = 1
        }
      } else if (this.$layout == ListLayoutType.FlowVertical) {
        if (this.$lineCount > 0) this.$curLineItemCount = this.$lineCount
        else {
          this.$curLineItemCount = Math.floor(
            (this.$scrollPane.viewHeight + this.$lineGap) / (this.$itemSize.y + this.$lineGap)
          )
          if (this.$curLineItemCount <= 0) this.$curLineItemCount = 1
        }
      } //pagination
        ...
        if (this.$lineCount > 0) this.$curLineItemCount2 = this.$lineCount
        else {
          this.$curLineItemCount2 = Math.floor(
            (this.$scrollPane.viewHeight + this.$lineGap) / (this.$itemSize.y + this.$lineGap)
          )
          if (this.$curLineItemCount2 <= 0) this.$curLineItemCount2 = 1
        }
      }
    }
    ...
    if (this.$realNumItems > 0) {
      let i: number
      let len: number =
        Math.ceil(this.$realNumItems / this.$curLineItemCount) * this.$curLineItemCount
      ...
      if (
        ...
        if (ch > 0) ch -= this.$lineGap

        if (this.$autoResizeItem) cw = this.$scrollPane.viewWidth
        else {
          for (i = 0; i < len2; i++) cw += this.$virtualItems[i].width + this.$columnGap
          if (cw > 0) cw -= this.$columnGap
        }
      } else if (
        this.$layout == ListLayoutType.SingleRow ||
        this.$layout == ListLayoutType.FlowVertical
        ...
        if (this.$autoResizeItem) ch = this.$scrollPane.viewHeight
        else {
          for (i = 0; i < len2; i++) ch += this.$virtualItems[i].height + this.$lineGap
          if (ch > 0) ch -= this.$lineGap
        }
      } else {
        const pageCount: number = Math.ceil(
          len / (this.$curLineItemCount * this.$curLineItemCount2)
        )
  ...
  private handleScroll1(forceUpdate: boolean): void {
    ...
    if (this.$enterCounter > 3) {
      console.warn(
        'this list view cannot be filled full as the itemRenderer function always returns an item with different size.'
      )
      return
    }
    ...
    if (newFirstIndex == this.$firstIndex && !forceUpdate) return
    ...
    const partSize: number =
      (this.$scrollPane.viewWidth - this.$columnGap * (this.$curLineItemCount - 1)) /
    ...
    while (curIndex < this.$realNumItems && (end || curY < max)) {
      ...
      if (ii.obj == null || forceUpdate) {
        if (this.itemProvider != null) {
          ...
          if (url == null) url = this.$defaultItem
      ...
      if (needRender) {
        if (
          this.$autoResizeItem &&
          (this.$layout == ListLayoutType.SingleColumn || this.$columnCount > 0)
        )
  ...
  private handleScroll2(forceUpdate: boolean): void {
    ...
    let i: number, j: number
    const partSize: number =
      (this.$scrollPane.viewHeight - this.$lineGap * (this.$curLineItemCount - 1)) /
      this.$curLineItemCount
    ...
    while (curIndex < this.$realNumItems && (end || curX < max)) {
      ...
      if (needRender) {
        if (
          this.$autoResizeItem &&
          (this.$layout == ListLayoutType.SingleRow || this.$lineCount > 0)
        )
  ...
  private handleScroll3(forceUpdate: boolean): void {
    ...
    let col: number
    let url: string = this.$defaultItem
    const partWidth: number =
      (this.$scrollPane.viewWidth - this.$columnGap * (this.$curLineItemCount - 1)) /
      this.$curLineItemCount
    const partHeight: number =
      (this.$scrollPane.viewHeight - this.$lineGap * (this.$curLineItemCount2 - 1)) /
    ...
    for (i = startIndex; i < lastIndex; i++) {
      if (i >= this.$realNumItems) continue
    ...
    for (i = startIndex; i < lastIndex; i++) {
      ...
      } else xx += ii.width + this.$columnGap
    }
  ...
  private handleAlign(contentWidth: number, contentHeight: number): void {
    ...
    if (contentWidth < this.viewWidth) {
      ...
      else if (this.$align == AlignType.Right) newOffsetX = this.viewWidth - contentWidth
    }

    if (newOffsetX != this.$alignOffset.x || newOffsetY != this.$alignOffset.y) {
      this.$alignOffset.setValues(newOffsetX, newOffsetY)
      if (this.$scrollPane != null) this.$scrollPane.adjustMaskContainer()
      else {
  ...
  protected updateBounds(): void {
    ...
    if (this.$layout == ListLayoutType.SingleColumn) {
      for (i = 0; i < cnt; i++) {
        ...
        if (curY != 0) curY += this.$lineGap
        child.y = curY
        if (this.$autoResizeItem) child.setSize(viewWidth, child.height, true)
      ...
      for (i = 0; i < cnt; i++) {
        ...
        if (curX != 0) curX += this.$columnGap
        child.x = curX
        if (this.$autoResizeItem) child.setSize(child.width, viewHeight, true)
      ...
      if (this.$autoResizeItem && this.$columnCount > 0) {
        for (i = 0; i < cnt; i++) {
          ...
          if (j == this.$columnCount || i == cnt - 1) {
            ...
            for (j = lineStart; j <= i; j++) {
              ...
              if (j < i) {
                child.setSize(
                  child.sourceWidth + Math.round(child.sourceWidth * ratio),
                  child.height,
                  true
                )
        ...
        for (i = 0; i < cnt; i++) {
          ...
          if (
            (this.$columnCount != 0 && j >= this.$columnCount) ||
      ...
      if (this.$autoResizeItem && this.$lineCount > 0) {
          ...
          if (j == this.$lineCount || i == cnt - 1) {
            ...
            for (j = lineStart; j <= i; j++) {
              ...
              if (j < i) {
                child.setSize(
                  child.width,
                  child.sourceHeight + Math.round(child.sourceHeight * ratio),
                  true
                )
        ...
        for (i = 0; i < cnt; i++) {
          ...
          if (
            (this.$lineCount != 0 && j >= this.$lineCount) ||
      ...
      if (this.$autoResizeItem && this.$lineCount > 0)
        eachHeight = Math.floor(
          (viewHeight - (this.$lineCount - 1) * this.$lineGap) / this.$lineCount
        )
          ...
          if (j == this.$columnCount || i == cnt - 1) {
            ...
            for (j = lineStart; j <= i; j++) {
              ...
              if (j < i) {
                child.setSize(
                  child.sourceWidth + Math.round(child.sourceWidth * ratio),
                  this.$lineCount > 0 ? eachHeight : child.height,
                  true
                )
                curX += Math.ceil(child.width) + this.$columnGap
              } else
            ...
            if (
              (this.$lineCount != 0 && k >= this.$lineCount) ||
  ...
  public setupBeforeAdd(xml: XmlNode): void {
    ...
    if (str) this.$layout = ParseListLayoutType(str)
    ...
    if (str) this.$lineGap = parseInt(str)

    str = xml.attributes.colGap
    if (str) this.$columnGap = parseInt(str)
    ...
    if (str) {
      ...
      else if (this.$layout == ListLayoutType.FlowVertical) this.$lineCount = parseInt(str)
    }

    str = xml.attributes.lineItemCount2
    if (str) this.$lineCount = parseInt(str)

    str = xml.attributes.selectionMode
    if (str) this.$selectionMode = ParseListSelectionMode(str)

    str = xml.attributes.defaultItem
    if (str) this.$defaultItem = str
    ...
    else this.$autoResizeItem = str == 'true'
    ...
    if (str) {
      ...
      if (this.$childrenRenderOrder == ListChildrenRenderOrder.Arch) {
        ...
        if (str) this.$apexIndex = parseInt(str)
      }
    }

    let col: XmlNode[] = xml.children
    col.forEach(cxml => {
      if (cxml.nodeName != 'item') return
      ...
      if (obj != null) {
        ...
        if (str && obj instanceof GButton) obj.selectedIcon = str
      }
    }, this)
  }
  ...
  public setupAfterAdd(xml: XmlNode): void {
    ...
    if (str) this.$selectionController = this.parent.getController(str)
  }
}
