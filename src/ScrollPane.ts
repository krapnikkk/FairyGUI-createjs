...
export class ScrollPane extends createjs.EventDispatcher {
  private static $easeTypeFunc: Function = (t: number, d: number): number => {
  ...
  private $viewSize: createjs.Point
  private $contentSize: createjs.Point
  private $overlapSize: createjs.Point
  private $pageSize: createjs.Point
  private $containerPos: createjs.Point
  private $beginTouchPos: createjs.Point
  private $lastTouchPos: createjs.Point
  private $lastTouchGlobalPos: createjs.Point
  ...
  private $tweenTime: createjs.Point
  private $tweenDuration: createjs.Point
  private $tweenStart: createjs.Point
  private $tweenChange: createjs.Point
  ...
  private static sHelperPoint: createjs.Point = new createjs.Point()
  private static sHelperRect: createjs.Rectangle = new createjs.Rectangle()
  private static sEndPos: createjs.Point = new createjs.Point()
  private static sOldChange: createjs.Point = new createjs.Point()

  public static TWEEN_DEFAULT_DURATION: number = 0.4
  ...
  public constructor(
    ...
    if (flags & ScrollPaneFlags.TouchEffect) this.$touchEffect = true
    else if (flags & ScrollPaneFlags.DisableTouchEffect) this.$touchEffect = false
    else this.$touchEffect = UIConfig.defaultScrollTouchEffect
    if (flags & ScrollPaneFlags.BounceEffect) this.$bouncebackEffect = true
    else if (flags & ScrollPaneFlags.DisableBounceEffect) this.$bouncebackEffect = false
    else this.$bouncebackEffect = UIConfig.defaultScrollBounceEffect
    ...
    if (scrollBarDisplay != ScrollBarDisplayType.Hidden) {
      if (this.$scrollType == ScrollType.Both || this.$scrollType == ScrollType.Vertical) {
        ...
        if (res) {
          ...
          if (!this.$vtScrollBar) throw new Error(`Cannot create scrollbar from ${res}`)
      ...
      if (this.$scrollType == ScrollType.Both || this.$scrollType == ScrollType.Horizontal) {
        ...
        if (res) {
          ...
          if (!this.$hzScrollBar) throw new Error(`Cannot create scrollbar from ${res}`)
      ...
      if (this.$scrollBarDisplayAuto) {
        ...
        if (this.$vtScrollBar) this.$vtScrollBar.displayObject.visible = false
        if (this.$hzScrollBar) this.$hzScrollBar.displayObject.visible = false
      }
    } else this.$mouseWheelEnabled = false

    if (headerRes) {
      this.$header = UIPackage.createObjectFromURL(headerRes) as GComponent
      if (this.$header == null) throw new Error(`Cannot create scrollPane.header from ${res}`)
    }

    if (footerRes) {
      this.$footer = UIPackage.createObjectFromURL(footerRes) as GComponent
      if (this.$footer == null) throw new Error(`Cannot create scrollPane.footer from ${res}`)
    }

    if (this.$header != null || this.$footer != null)
      this.$refreshBarAxis =
        this.$scrollType == ScrollType.Both || this.$scrollType == ScrollType.Vertical ? 'y' : 'x'
  ...
  public dispose(): void {
    if (this.$tweening != 0) GTimer.inst.remove(this.tweenUpdate, this)

    this.$pageController = null

    if (this.$hzScrollBar != null) this.$hzScrollBar.dispose()
    if (this.$vtScrollBar != null) this.$vtScrollBar.dispose()
    if (this.$header != null) this.$header.dispose()
    if (this.$footer != null) this.$footer.dispose()
    ...
    Decls.GRoot.inst.nativeStage.off(InteractiveEvents.Click, this.$clickEvent)
    ...
    this.$owner.off(DisplayObjectEvent.MOUSE_WHEEL, this.$mouseWheel)
  }
  ...
  public set scrollSpeed(val: number) {
    ...
    if (this.$scrollSpeed == 0) this.$scrollSpeed = UIConfig.defaultScrollSpeed
    this.$mouseWheelSpeed = this.$scrollSpeed * 2
  }
  ...
  public setPosX(value: number, ani: boolean = false): void {
    ...
    if (this.$loop == 1) value = this.loopCheckingNewPos(value, 'x')
  ...
  public setPosY(value: number, ani: boolean = false): void {
    ...
    if (this.$loop == 1) value = this.loopCheckingNewPos(value, 'y')
  ...
  public get currentPageX(): number {
    ...
    if (this.$xPos - page * this.$pageSize.x > this.$pageSize.x * 0.5) page++

    return page
  }

  public set currentPageX(value: number) {
    if (this.$pageMode && this.$overlapSize.x > 0) this.setPosX(value * this.$pageSize.x, false)
  }
  ...
  public get currentPageY(): number {
    ...
    if (this.$yPos - page * this.$pageSize.y > this.$pageSize.y * 0.5) page++

    return page
  }

  public set currentPageY(value: number) {
    if (this.$pageMode && this.$overlapSize.y > 0) this.setPosY(value * this.$pageSize.y, false)
  }
  ...
  public scrollUp(ratio: number = 1, ani: boolean = false): void {
    if (this.$pageMode) this.setPosY(this.$yPos - this.$pageSize.y * ratio, ani)
    else this.setPosY(this.$yPos - this.$scrollSpeed * ratio, ani)
  }
  ...
  public scrollLeft(ratio: number = 1, ani: boolean = false): void {
    if (this.$pageMode) this.setPosX(this.$xPos - this.$pageSize.x * ratio, ani)
    else this.setPosX(this.$xPos - this.$scrollSpeed * ratio, ani)
  }
  ...
  public scrollToView(target: Object, ani: boolean = false, snapToFirst: boolean = false): void {
    ...
    if (target instanceof GObject) {
      if (target.parent != this.$owner) {
        target.parent.localToGlobalRect(
          ...
          target.width,
          target.height,
          ScrollPane.sHelperRect
        )
        rect = this.$owner.globalToLocalRect(
          ScrollPane.sHelperRect.x,
          ScrollPane.sHelperRect.y,
          ScrollPane.sHelperRect.width,
          ScrollPane.sHelperRect.height,
          ScrollPane.sHelperRect
        )
      } else {
    ...
    } else rect = target as createjs.Rectangle
    ...
    if (this.$overlapSize.y > 0) {
      ...
      if (snapToFirst || rect.y <= this.$yPos || rect.height >= this.$viewSize.y) {
        ...
        else this.setPosY(rect.y + rect.height - this.$viewSize.y, ani)
      }
    }
    if (this.$overlapSize.x > 0) {
      ...
      if (snapToFirst || rect.x <= this.$xPos || rect.width >= this.$viewSize.x) {
        ...
        else this.setPosX(rect.x + rect.width - this.$viewSize.x, ani)
      }
    }

    if (!ani && this.$needRefresh) this.refresh()
  }

  public isChildInView(obj: GObject): boolean {
    if (this.$overlapSize.y > 0) {
      var dist: number = obj.y + this.$container.y
      if (dist < -obj.height || dist > this.$viewSize.y) return false
    }

    if (this.$overlapSize.x > 0) {
      dist = obj.x + this.$container.x
      if (dist < -obj.width || dist > this.$viewSize.x) return false
    }
  ...
  public lockHeader(size: number): void {
    if (this.$headerLockedSize == size) return

    this.$headerLockedSize = size

    if (
      !this.$refreshEventDispatching &&
      (<IndexedObject>this.$container)[this.$refreshBarAxis] >= 0
    ) {
      this.$tweenStart.setValues(this.$container.x, this.$container.y)
      this.$tweenChange.setValues(0, 0)
      ;(<IndexedObject>this.$tweenChange)[this.$refreshBarAxis] =
        this.$headerLockedSize - (<IndexedObject>this.$tweenStart)[this.$refreshBarAxis]
      this.$tweenDuration.setValues(
        ScrollPane.TWEEN_DEFAULT_DURATION,
        ScrollPane.TWEEN_DEFAULT_DURATION
      )
  ...
  public lockFooter(size: number): void {
    if (this.$footerLockedSize == size) return

    this.$footerLockedSize = size

    if (
      !this.$refreshEventDispatching &&
      (<IndexedObject>this.$container)[this.$refreshBarAxis] <=
        -(<IndexedObject>this.$overlapSize)[this.$refreshBarAxis]
    ) {
      this.$tweenStart.setValues(this.$container.x, this.$container.y)
      this.$tweenChange.setValues(0, 0)
      ...
      if (max == 0)
        max = Math.max(
          (<IndexedObject>this.$contentSize)[this.$refreshBarAxis] +
            this.$footerLockedSize -
            (<IndexedObject>this.$viewSize)[this.$refreshBarAxis],
          0
        )
      else max += this.$footerLockedSize
      ;(<IndexedObject>this.$tweenChange)[this.$refreshBarAxis] =
        -max - (<IndexedObject>this.$tweenStart)[this.$refreshBarAxis]
      this.$tweenDuration.setValues(
        ScrollPane.TWEEN_DEFAULT_DURATION,
        ScrollPane.TWEEN_DEFAULT_DURATION
      )
  ...
  handleControllerChanged(c: Controller): void {
    if (this.$pageController == c) {
      if (this.$scrollType == ScrollType.Horizontal) this.currentPageX = c.selectedIndex
      else this.currentPageY = c.selectedIndex
    }
  }
  ...
  private updatePageController(): void {
    if (this.$pageController != null && !this.$pageController.$updating) {
      let index: number
      if (this.$scrollType == ScrollType.Horizontal) index = this.currentPageX
  ...
  adjustMaskContainer(): void {
    ...
    else x = Math.floor(this.$owner.margin.left)
    ...
    } else if (this.$alignContainer) this.$alignContainer.set({ x: 0, y: 0 })
  }
  ...
  public setSize(width: number, height: number): void {
    ...
    if (this.$hzScrollBar) {
      ...
      if (this.$vtScrollBar && !this.$vScrollNone) {
        ...
        else this.$hzScrollBar.x = this.$scrollBarMargin.left
      } else {
    ...
    if (this.$vtScrollBar) {
      if (!this.$displayOnLeft) this.$vtScrollBar.x = width - this.$vtScrollBar.width
      if (this.$hzScrollBar)
    ...
    if (this.$hzScrollBar && !this.$hScrollNone) this.$viewSize.y -= this.$hzScrollBar.height
    if (this.$vtScrollBar && !this.$vScrollNone) this.$viewSize.x -= this.$vtScrollBar.width
  ...
  public setContentSize(w: number, h: number): void {
    if (this.$contentSize.x == w && this.$contentSize.y == h) return
  ...
  changeContentSizeOnScrolling(
    ...
    if (this.$pageMode) this.updatePageController()
  }
  ...
  private handleSizeChanged(onScrolling: boolean = false): void {
    ...
    if (this.$vtScrollBar) {
      if (this.$viewSize.y < this.$vtScrollBar.minSize)
        ...
        if (this.$contentSize.y == 0) this.$vtScrollBar.displayPerc = 0
        else this.$vtScrollBar.displayPerc = Math.min(1, this.$viewSize.y / this.$contentSize.y)
      }
    }
    if (this.$hzScrollBar) {
      if (this.$viewSize.x < this.$hzScrollBar.minSize)
        ...
        if (this.$contentSize.x == 0) this.$hzScrollBar.displayPerc = 0
        else this.$hzScrollBar.displayPerc = Math.min(1, this.$viewSize.x / this.$contentSize.x)
      }
    }

    const rect: createjs.Rectangle = this.$maskContainer.scrollRect
    if (rect) {
    ...
    if (this.$refreshBarAxis != null) {
      ...
      if (this.$header != null) {
        if (this.$refreshBarAxis == 'x') this.$header.height = this.$viewSize.y
        else this.$header.width = this.$viewSize.x
      }

      if (this.$footer != null) {
        if (this.$refreshBarAxis == 'y') this.$footer.height = this.$viewSize.y
        else this.$footer.width = this.$viewSize.x
      }
    } else {
      this.$container.set({
        x: NumberUtil.clamp(this.$container.x, -this.$overlapSize.x, 0),
  ...
  private posChanged(ani: boolean): void {
    if (this.$aniFlag == 0) this.$aniFlag = ani ? 1 : -1
    else if (this.$aniFlag == 1 && !ani) this.$aniFlag = -1
  ...
  private refresh(): void {
    ...
    let evt = new createjs.Event(ScrollEvent.SCROLL, true, false)
  ...
  private refresh2(): void {
    if (this.$aniFlag == 1 && !this.$isDragging) {
      ...
      if (this.$overlapSize.x > 0) posX = -Math.floor(this.$xPos)
      else {
        if (this.$container.x != 0) this.$container.x = 0
        posX = 0
      }
      if (this.$overlapSize.y > 0) posY = -Math.floor(this.$yPos)
      else {
      ...
      if (posX != this.$container.x || posY != this.$container.y) {
        ...
        this.$tweenTime.setValues(0, 0)
        this.$tweenDuration.setValues(
          ScrollPane.TWEEN_MANUALLY_SET_DURATION,
          ScrollPane.TWEEN_MANUALLY_SET_DURATION
        )
        this.$tweenStart.setValues(this.$container.x, this.$container.y)
        this.$tweenChange.setValues(posX - this.$tweenStart.x, posY - this.$tweenStart.y)
        GTimer.inst.addLoop(1, this.tweenUpdate, this)
      } else if (this.$tweening != 0) this.killTween()
    } else {
      if (this.$tweening != 0) this.killTween()
  ...
  private syncScrollBar(end: boolean = false): void {
    if (this.$vtScrollBar != null) {
      ...
      if (this.$scrollBarDisplayAuto) this.showScrollBar(!end)
    }
    if (this.$hzScrollBar != null) {
    ...
    if (end) this.$maskContainer.mouseEnabled = true
  }
  ...
  private $mouseDown(e: any): void {
    ...
    const globalMouse: createjs.Point = isMobile.any
      ? this.$owner.globalToLocal(e.localX, e.localY)
      : this.$owner.globalToLocal(
          Decls.GRoot.globalMouseStatus.mouseX,
          Decls.GRoot.globalMouseStatus.mouseY,
    ...
    this.$clickEvent = Decls.GRoot.inst.nativeStage.on(InteractiveEvents.Click, this.$click, this)
  }
  ...
  private $mouseMove(): void {
    ...
    if (
      (ScrollPane.draggingPane != null && ScrollPane.draggingPane != this) ||
    ...
    const globalMouse: createjs.Point = this.$owner.globalToLocal(
      Decls.GRoot.globalMouseStatus.mouseX,
      Decls.GRoot.globalMouseStatus.mouseY,
      ScrollPane.sHelperPoint
    )
    ...
    if (sv) {
      if (newPosY > 0) {
        if (!this.$bouncebackEffect) this.$container.y = 0
          ...
          this.$container.y = Math.floor(
            Math.min(newPosY * 0.5, this.$viewSize.y * ScrollPane.PULL_DIST_RATIO)
          )
      } else if (newPosY < -this.$overlapSize.y) {
        if (!this.$bouncebackEffect) this.$container.y = -this.$overlapSize.y
        else if (this.$footer != null && this.$footer.height > 0)
          //TODO: height -> maxHeight
          this.$container.y = Math.floor(
            Math.max((newPosY + this.$overlapSize.y) * 0.5, -this.$footer.height) -
              this.$overlapSize.y
          )
        else
          this.$container.y = Math.floor(
            Math.max(
              (newPosY + this.$overlapSize.y) * 0.5,
              -this.$viewSize.y * ScrollPane.PULL_DIST_RATIO
            ) - this.$overlapSize.y
          )
      } else this.$container.y = newPosY
    }

    if (sh) {
      if (newPosX > 0) {
        if (!this.$bouncebackEffect) this.$container.x = 0
          ...
          this.$container.x = Math.floor(
            Math.min(newPosX * 0.5, this.$viewSize.x * ScrollPane.PULL_DIST_RATIO)
          )
      } else if (newPosX < 0 - this.$overlapSize.x) {
        if (!this.$bouncebackEffect) this.$container.x = -this.$overlapSize.x
        else if (this.$footer != null && this.$footer.width > 0)
          //TODO: width -> maxWidth
          this.$container.x = Math.floor(
            Math.max((newPosX + this.$overlapSize.x) * 0.5, -this.$footer.width) -
              this.$overlapSize.x
          )
        else
          this.$container.x = Math.floor(
            Math.max(
              (newPosX + this.$overlapSize.x) * 0.5,
              -this.$viewSize.x * ScrollPane.PULL_DIST_RATIO
            ) - this.$overlapSize.x
          )
      } else this.$container.x = newPosX
    }

    //update acceleration
    const frameRate: number = createjs.Ticker.framerate
    ...
    if (deltaTime != 0) {
      ...
      this.$velocity.x = NumberUtil.lerp(
        this.$velocity.x,
        (deltaPositionX * 60) / frameRate / deltaTime,
        deltaTime * 10
      )
      this.$velocity.y = NumberUtil.lerp(
        this.$velocity.y,
        (deltaPositionY * 60) / frameRate / deltaTime,
        deltaTime * 10
      )
    }
    ...
    if (deltaPositionX != 0) this.$velocityScale = Math.abs(deltaGlobalPositionX / deltaPositionX)
  ...
  private $mouseUp(): void {
    Decls.GRoot.inst.nativeStage.off(InteractiveEvents.Move, this.$mouseMoveEvent)
    Decls.GRoot.inst.nativeStage.off(InteractiveEvents.Up, this.$mouseMoveEvent)
    Decls.GRoot.inst.nativeStage.off(InteractiveEvents.Click, this.$clickEvent)
    ...
    if (flag) {
      this.$tweenChange.setValues(
        ScrollPane.sEndPos.x - this.$tweenStart.x,
        ScrollPane.sEndPos.y - this.$tweenStart.y
      )
      if (
        this.$tweenChange.x < -UIConfig.touchDragSensitivity ||
        this.$tweenChange.y < -UIConfig.touchDragSensitivity
      ) {
        this.$refreshEventDispatching = true
        let evt = new createjs.Event(ScrollEvent.PULL_DOWN_RELEASE, true, false)
        this.dispatchEvent(evt, this)
        ...
        this.$refreshEventDispatching = true
        let evt = new createjs.Event(ScrollEvent.PULL_UP_RELEASE, true, false)
        this.dispatchEvent(evt, this)
        this.$refreshEventDispatching = false
      }
      ...
      if (
        this.$headerLockedSize > 0 &&
        (<IndexedObject>ScrollPane.sEndPos)[this.$refreshBarAxis] == 0
      ) {
        ...
        this.$footerLockedSize > 0 &&
        (<IndexedObject>ScrollPane.sEndPos)[this.$refreshBarAxis] ==
          -(<IndexedObject>this.$overlapSize)[this.$refreshBarAxis]
      ) {
        var max: number = (<IndexedObject>this.$overlapSize)[this.$refreshBarAxis]
        if (max == 0)
      ...
      ScrollPane.sOldChange.setValues(
        ScrollPane.sEndPos.x - this.$tweenStart.x,
        ScrollPane.sEndPos.y - this.$tweenStart.y
      )
      ...
      if (this.$pageMode || this.$snapToItem) this.alignPosition(ScrollPane.sEndPos, true)

      this.$tweenChange.x = ScrollPane.sEndPos.x - this.$tweenStart.x
      this.$tweenChange.y = ScrollPane.sEndPos.y - this.$tweenStart.y
      if (this.$tweenChange.x == 0 && this.$tweenChange.y == 0) {
        if (this.$scrollBarDisplayAuto) this.showScrollBar(false)
        return
      }
  ...
  private $mouseWheel(evt: any): void {
    ...
    if (this.$overlapSize.x > 0 && this.$overlapSize.y == 0) {
      if (this.$pageMode) this.setPosX(this.$xPos + this.$pageSize.x * delta, false)
      else this.setPosX(this.$xPos + this.$mouseWheelSpeed * delta, false)
    } else {
      if (this.$pageMode) this.setPosY(this.$yPos + this.$pageSize.y * delta, false)
      else this.setPosY(this.$yPos + this.$mouseWheelSpeed * delta, false)
    }
  }
  ...
  private showScrollBar(visible: boolean): void {
    ...
    } else GTimer.inst.add(500, 1, this.setScrollBarVisible, this, visible)
  }
  ...
  private loopCheckingCurrent(): boolean {
    ...
    if (changed) this.$container.set({ x: Math.floor(-this.$xPos), y: Math.floor(-this.$yPos) })

    return changed
  }

  private loopCheckingTarget(endPos: createjs.Point): void {
    if (this.$loop == 1) this.loopCheckingTarget2(endPos, 'x')

    if (this.$loop == 2) this.loopCheckingTarget2(endPos, 'y')
  }

  private loopCheckingTarget2(endPos: createjs.Point, axis: string): void {
    let halfSize: number
    let tmp: number
  ...
  private loopCheckingNewPos(value: number, axis: string): number {
    if ((<IndexedObject>this.$overlapSize)[axis] == 0) return value
    ...
    if (value < 0.001) {
      ...
      if (value > pos) {
        ...
        pos = NumberUtil.clamp(pos + v, 0, (<IndexedObject>this.$overlapSize)[axis])
        changed = true
      }
    ...
    if (changed) {
      if (axis == 'x') this.$container.x = -Math.floor(pos)
      else this.$container.y = -Math.floor(pos)
    }

    return value
  }

  private alignPosition(pos: createjs.Point, inertialScrolling: boolean): void {
    if (this.$pageMode) {
      pos.x = this.alignByPage(pos.x, 'x', inertialScrolling)
      pos.y = this.alignByPage(pos.y, 'y', inertialScrolling)
    } else if (this.$snapToItem) {
      var pt: createjs.Point = this.$owner.getSnappingPosition(
        -pos.x,
        -pos.y,
        ScrollPane.sHelperPoint
      )
      if (pos.x < 0 && pos.x > -this.$overlapSize.x) pos.x = -pt.x
      if (pos.y < 0 && pos.y > -this.$overlapSize.y) pos.y = -pt.y
    }
  }
  ...
  private alignByPage(pos: number, axis: string, inertialScrolling: boolean): number {
    ...
    if (pos > 0) page = 0
      ...
      page =
        Math.ceil(
          (<IndexedObject>this.$contentSize)[axis] / (<IndexedObject>this.$pageSize)[axis]
        ) - 1
    else {
      page = Math.floor(-pos / (<IndexedObject>this.$pageSize)[axis])
      var change: number = inertialScrolling
        ? pos - (<IndexedObject>this.$containerPos)[axis]
        : pos - (<IndexedObject>this.$container)[axis]
      var testPageSize: number = Math.min(
        (<IndexedObject>this.$pageSize)[axis],
        (<IndexedObject>this.$contentSize)[axis] -
          (page + 1) * (<IndexedObject>this.$pageSize)[axis]
      )
    ...
    if (inertialScrolling) {
      ...
      else oldPage = Math.floor(-oldPos / (<IndexedObject>this.$pageSize)[axis])
      var startPage: number = Math.floor(
        -(<IndexedObject>this.$containerPos)[axis] / (<IndexedObject>this.$pageSize)[axis]
      )
  ...
  private updateTargetAndDuration(orignPos: createjs.Point, resultPos: createjs.Point): void {
  ...
  private updateTargetAndDuration2(pos: number, axis: string): number {
    ...
    if (pos > 0) pos = 0
      ...
      if (isMobile.any)
        v2 *=
          Math.max(
            Decls.GRoot.inst.stageWrapper.designWidth,
            Decls.GRoot.inst.stageWrapper.designHeight
          ) / Math.max(Decls.GRoot.inst.stageWidth, Decls.GRoot.inst.stageHeight)
    ...
    if (duration < ScrollPane.TWEEN_DEFAULT_DURATION) duration = ScrollPane.TWEEN_DEFAULT_DURATION
  ...
  private fixDuration(axis: string, oldChange: number): void {
    if (
      ...
      Math.abs((<IndexedObject>this.$tweenChange)[axis]) >= Math.abs(oldChange)
    )
      return

    let newDuration: number =
      Math.abs((<IndexedObject>this.$tweenChange)[axis] / oldChange) *
      (<IndexedObject>this.$tweenDuration)[axis]
  ...
  private killTween(): void {
    ...
    let evt = new createjs.Event(ScrollEvent.SCROLL_END, true, false)
    this.dispatchEvent(evt, this)
  }

  private checkRefreshBar(): void {
    if (this.$header == null && this.$footer == null) return
    ...
    if (this.$header != null) {
      if (pos > 0) {
        ...
        const pt: createjs.Point = ScrollPane.sHelperPoint
    ...
    if (this.$footer != null) {
      ...
      if (pos < -max || (max == 0 && this.$footerLockedSize > 0)) {
        ...
        if (max > 0)
          (<IndexedObject>pt)[this.$refreshBarAxis] =
            pos + (<IndexedObject>this.$contentSize)[this.$refreshBarAxis]
        else
          (<IndexedObject>pt)[this.$refreshBarAxis] = Math.max(
            Math.min(
              pos + (<IndexedObject>this.$viewSize)[this.$refreshBarAxis],
              (<IndexedObject>this.$viewSize)[this.$refreshBarAxis] - this.$footerLockedSize
            ),
            (<IndexedObject>this.$viewSize)[this.$refreshBarAxis] -
              (<IndexedObject>this.$contentSize)[this.$refreshBarAxis]
          )
        ...
        if (max > 0) (<IndexedObject>pt)[this.$refreshBarAxis] = -max - pos
        else
          (<IndexedObject>pt)[this.$refreshBarAxis] =
            (<IndexedObject>this.$viewSize)[this.$refreshBarAxis] -
            (<IndexedObject>this.$footer)[this.$refreshBarAxis]
  ...
  private tweenUpdate(): void {
    ...
    if (this.$tweening == 2) {
      if (this.$overlapSize.x > 0) this.$xPos = NumberUtil.clamp(-nx, 0, this.$overlapSize.x)
      if (this.$overlapSize.y > 0) this.$yPos = NumberUtil.clamp(-ny, 0, this.$overlapSize.y)
    ...
    if (this.$tweenChange.x == 0 && this.$tweenChange.y == 0) {
      ...
      let scrollEvent = new createjs.Event(ScrollEvent.SCROLL, true, false)
      this.dispatchEvent(scrollEvent, this)
      let scrollEndEvent = new createjs.Event(ScrollEvent.SCROLL_END, true, false)
  ...
  private runTween(axis: string): number {
    const delta: number = createjs.Ticker.interval
    let newValue: number
    if ((<IndexedObject>this.$tweenChange)[axis] != 0) {
      ;(<IndexedObject>this.$tweenTime)[axis] += delta * createjs.Ticker.framerate
      if ((<IndexedObject>this.$tweenTime)[axis] >= (<IndexedObject>this.$tweenDuration)[axis]) {
        newValue =
          (<IndexedObject>this.$tweenStart)[axis] + (<IndexedObject>this.$tweenChange)[axis]
        ;(<IndexedObject>this.$tweenChange)[axis] = 0
      } else {
        const ratio: number = ScrollPane.$easeTypeFunc(
          (<IndexedObject>this.$tweenTime)[axis],
          (<IndexedObject>this.$tweenDuration)[axis]
        )
        newValue =
          (<IndexedObject>this.$tweenStart)[axis] +
          Math.floor((<IndexedObject>this.$tweenChange)[axis] * ratio)
      }
      ...
      if (this.$tweening == 2 && this.$bouncebackEffect) {
        if (
          (newValue > 20 + threshold1 && (<IndexedObject>this.$tweenChange)[axis] > 0) ||
          (newValue > threshold1 && (<IndexedObject>this.$tweenChange)[axis] == 0)
        ) {
          //start to bounce
          ...
          (newValue < threshold2 - 20 && (<IndexedObject>this.$tweenChange)[axis] < 0) ||
    ...
    } else newValue = (<IndexedObject>this.$container)[axis]
