...
import { ParseEaseType, ParseProgressTitleType, ProgressTitleType } from './config/Definitions'
...
export class GProgressBar extends GComponent {
  ...
  public update(val: number): void {
    ...
    if (!this.$reverse) {
      if (this.$barObjectH) this.$barObjectH.width = fullWidth * percent
      if (this.$barObjectV) this.$barObjectV.height = fullHeight * percent
    } else {
      if (this.$barObjectH) {
  ...
  protected handleSizeChanged(): void {
    ...
    if (!this.$inProgressBuilding) this.update(this.$value)
  }
