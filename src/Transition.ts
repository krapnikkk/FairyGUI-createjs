...
export class Transition {
  ...
  private $ownerVisibleChanged(event: createjs.Event): void {
    let vis: boolean = event.data
  ...
  public set autoPlay(value: boolean) {
    if (this.$autoPlay != value) {
      ...
      if (this.$autoPlay) {
        ...
        if (!this.$owner.onStage) this.stop(false, true)
      }
    }
  }
  ...
  public play(...args: any[]): void {
    ...
    } else this.$play(args[0], args[1], args[2], args[3] || 1, args[4] || 0, false)
  }
  ...
  public playReverse(...args: any[]): void {
    ...
    } else this.$play(args[0], args[1], args[2], args[3] || 1, args[4] || 0, true)
  }
    ...
    if (item.type == TransitionActionType.Transition) {
      ...
      if (setToComplete) {
        if (item.tween) {
          ...
          else this.applyValue(item, this.$reversed ? item.endValue : item.startValue)
        } else if (item.type != TransitionActionType.Sound) this.applyValue(item, item.value)
      }
    }
  }
  ...
  public dispose(): void {
    ...
    this.$owner.off(DisplayObjectEvent.VISIBLE_CHANGED, this.$ownerVisibleChanged)

    this.$playing = false
    this.$items.forEach(item => {
      if (item.target == null || item.completed) return
      ...
      } else if (item.type == TransitionActionType.Shake) GTimer.inst.remove(item.$shake, item)
    }, this)
  }
  ...
  public setValue(label: string, ...args: any[]) {
    this.$items.forEach(item => {
      if (item.label == null && item.label2 == null) return

      let value: TransitionValue

      if (item.label == label) {
        if (item.tween) value = item.startValue
        else value = item.value
      } else if (item.label2 == label) value = item.endValue
      else return
      ...
      switch (item.type) {
        ...
        case TransitionActionType.Animation:
          value.i = parseInt(args[0])
          if (args.length > 1) value.b = args[1]
          break
        ...
        case TransitionActionType.Sound:
          value.s = args[0]
          if (args.length > 1) value.f1 = parseFloat(args[1])
          break
        case TransitionActionType.Transition:
          value.s = args[0]
          if (args.length > 1) value.i = parseInt(args[1])
          break
        case TransitionActionType.Shake:
          value.f1 = parseFloat(args[0])
          if (args.length > 1) value.f2 = parseFloat(args[1])
          break
  ...
  public setTarget(label: string, newTarget: GObject) {
    this.$items.forEach(item => {
      if (item.label == label) item.targetId = newTarget.id
    }, this)
  }

  public setDuration(label: string, value: number) {
    this.$items.forEach(item => {
      if (item.tween && item.label == label) item.duration = value
    }, this)
  }
  ...
  private internalPlay(delay: number = 0): void {
    ...
    this.$items.forEach(item => {
      if (item.targetId) item.target = this.$owner.getChildById(item.targetId)
      else item.target = this.$owner
      ...
      this.disposeTween(item)
      if (item.tween) {
        if (this.$reversed) startTime = delay + this.$maxTime - item.time - item.duration
        else startTime = delay + item.time
        if (startTime > 0) {
          this.$totalTasks++
        ...
        } else this.startTween(item)
      } else {
        if (this.$reversed) startTime = delay + this.$maxTime - item.time
        else startTime = delay + item.time
        if (startTime <= 0) this.applyValue(item, item.value)
        else {
          this.$totalTasks++
          item.completed = false
          item.tweener = createjs.Tween.get(item.value)
      ...
      case TransitionActionType.Size:
        if (item.type == TransitionActionType.XY) {
          if (item.target == this.$owner) {
            if (!startValue.b1) startValue.f1 = 0
            if (!startValue.b2) startValue.f2 = 0
          } else {
            if (!startValue.b1) startValue.f1 = item.target.x
            if (!startValue.b2) startValue.f2 = item.target.y
          }
        } else {
          if (!startValue.b1) startValue.f1 = item.target.width
          if (!startValue.b2) startValue.f2 = item.target.height
        }
        item.value.f1 = startValue.f1
        item.value.f2 = startValue.f2

        if (!endValue.b1) endValue.f1 = item.value.f1
        if (!endValue.b2) endValue.f2 = item.value.f2
  ...
  private startTween(item: TransitionItem) {
    ...
    } else completeHandler = Binder.create(this.$tweenComplete, this, item)
    ...
    if (item.hook != null) item.hook.call(item.hookObj)
  }
  ...
  private $tweenComplete(event: any, item: TransitionItem) {
    ...
    this.$totalTasks--
    item.completed = true
    if (item.hook2 != null) item.hook2.call(item.hook2Obj)
    this.checkAllComplete()
  }
  ...
  private $tweenRepeatComplete(event: any, item: TransitionItem) {
    ...
    if (item.repeat == -1 || item.tweenTimes < item.repeat + 1) {
      ...
      if (item.yoyo) {
        if (this.$reversed) reversed = item.tweenTimes % 2 == 0
        else reversed = item.tweenTimes % 2 == 1
      } else reversed = this.$reversed
    ...
    } else this.$tweenComplete(null, item)
  }
  ...
  private checkAllComplete() {
    if (this.$playing && this.$totalTasks == 0) {
      if (this.$totalTimes < 0) {
        ...
        this.$totalTimes--
        if (this.$totalTimes > 0) GTimer.inst.callLater(this.internalPlay, this, 0)
        else {
          this.$playing = false
          this.$items.forEach(item => {
            if (item.target != null) {
    ...
    switch (item.type) {
      case TransitionActionType.XY:
        if (item.target == this.$owner) {
          let f1: number = 0,
            f2: number = 0
          if (!value.b1) f1 = item.target.x
          else f1 = value.f1 + this.$ownerBaseX
          if (!value.b2) f2 = item.target.y
          else f2 = value.f2 + this.$ownerBaseY
          item.target.setXY(f1, f2)
        } else {
          if (!value.b1) value.f1 = item.target.x
          if (!value.b2) value.f2 = item.target.y
      ...
      case TransitionActionType.Skew:
        ...
        break
      case TransitionActionType.Color:
        if (isColorGear(item.target)) item.target.color = value.c
        break
      case TransitionActionType.Animation:
        if (isAnimationGear(item.target)) {
          if (!value.b1) value.i = item.target.frame
      ...
      case TransitionActionType.Transition:
        ...
        if (trans != null) {
          if (value.i == 0) trans.stop(false, true)
          else if (trans.playing) trans.$totalTimes = value.i == -1 ? Number.MAX_VALUE : value.i
          else {
            item.completed = false
            this.$totalTasks++
            if (this.$reversed) trans.playReverse(this.$playTransComplete, this, item, item.value.i)
            else trans.play(this.$playTransComplete, this, item, item.value.i)
          }
        }
        break
      case TransitionActionType.Sound:
        if (this.$playing && item.time >= this.$startTime) {
          if (!value.audioClip) {
            ...
            if (pi) value.audioClip = pi.owner.getItemAsset(pi) as createjs.AbstractSoundInstance
          }
          if (value.audioClip) {
            Decls.GRoot.inst.playOneShotSound(value.audioClip, value.f1)
          }
        }
        break
      case TransitionActionType.Shake:
  ...
  $shakeItem(item: TransitionItem, elapsedMS: number) {
    ...
    item.target.$gearLocked = true
    item.target.setXY(
      item.target.x - item.startValue.f1 + rx,
      item.target.y - item.startValue.f2 + ry
    )
    item.target.$gearLocked = false
  ...
  public setup(xml: XmlNode) {
    ...
    col.forEach(cxml => {
      ...
      if (item.tween) {
        ...
        if (item.time + item.duration > this.$maxTime) this.$maxTime = item.time + item.duration
        str = cxml.attributes.ease
        if (str) item.easeType = ParseEaseType(str)
        str = cxml.attributes.repeat
        if (str) item.repeat = parseInt(str)
        ...
        if (item.time > this.$maxTime) this.$maxTime = item.time
    ...
    switch (type) {
      ...
      case TransitionActionType.Sound:
        ...
        if (arr.length > 1) {
          ...
          if (intv == 0 || intv == 100) value.f1 = 1
          else value.f1 = intv / 100
        } else value.f1 = 1
        break
      case TransitionActionType.Transition:
        arr = str.split(',')
        value.s = arr[0]
        if (arr.length > 1) value.i = parseInt(arr[1])
        else value.i = 1
        break
      case TransitionActionType.Shake:
        arr = str.split(',')
...
class TransitionValue {
  ...
  public audioClip?: createjs.AbstractSoundInstance
}
