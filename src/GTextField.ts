...
export class GTextField extends GObject implements IColorGear, IColorableTitle {
  ...
  protected $offset: createjs.Point = new createjs.Point()
  protected $color: string
  protected $singleLine: boolean = true

  protected $text: string = ''
  protected $fontProperties: FontMetrics
  ...
  private switchBitmapMode(val: boolean): void {
    if (val && this.displayObject == this.$textField) {
      if (this.$btContainer == null) this.$btContainer = new UIContainer(this)
  ...
  protected setText(value: string): void {
    ...
    if (this.parent && this.parent.$inProgressBuilding) this.renderNow()
    else this.render()
  }
  ...
  public get lineHeight(): number {
    if (this.$style.lineHeight > 0) return this.$style.lineHeight
  ...
  public set font(value: string) {
    if (this.$font != value) {
      ...
      else this.$style.fontFamily = this.$font || UIConfig.defaultFont
      this.render()
    }
  }
  ...
  public set verticalAlign(value: VertAlignType) {
    if (this.$verticalAlign != value) {
      ...
      if (!this.$inProgressBuilding) this.layoutAlign()
    }
  }
  ...
  public set bold(value: boolean) {
    ...
    if (this.$style.fontWeight != v) {
      this.$style.fontWeight = v as TextStyleFontWeight
      this.render()
    }
  }
  ...
  public set variant(v: string) {
    if (this.$style.fontVariant != v) {
      this.$style.fontVariant = v as TextStyleFontVariant
      this.render()
    }
  }
  ...
  public set italic(value: boolean) {
    ...
    if (this.$style.fontStyle != v) {
      this.$style.fontStyle = v as TextStyleFontStyle
      this.render()
    }
  }
  ...
  public set stroke(value: number) {
    if (this.$style.strokeThickness != value) this.$style.strokeThickness = value
  }
  ...
  public set strokeColor(value: string) {
    if (this.$style.stroke != value) this.$style.stroke = value
  }
  ...
  public get textWidth(): number {
    if (this.$requireRender) this.renderNow()
  ...
  public ensureSizeCorrect(): void {
    if (this.$sizeDirty && this.$requireRender) this.renderNow()
  }
  ...
  protected render(): void {
    ...
    if (!this.$sizeDirty && (this.$widthAutoSize || this.$heightAutoSize)) {
      this.$sizeDirty = true
      let evt = new createjs.Event(DisplayObjectEvent.SIZE_DELAY_CHANGE, true, false)
  ...
  protected renderNow(updateBounds: boolean = true): void {
    ...
    this.$textField.width = this.$textField.style.wordWrapWidth =
      wordWrap || this.autoSize == AutoSizeType.None ? Math.ceil(this.width) : 10000
    ...
    if (this.$textWidth > 0) this.$textWidth += GTextField.GUTTER_X * 2 //margin gap
    this.$textHeight = Math.ceil(this.$textField.textHeight)
    if (this.$textHeight > 0) this.$textHeight += GTextField.GUTTER_Y * 2 //margin gap
    ...
    if (this.autoSize == AutoSizeType.Shrink) {
      ...
      if (this.$heightAutoSize) {
        h = this.$textHeight
        if (this.$textField.height != this.$textHeight) this.$textField.height = this.$textHeight
      } else {
        h = this.height
        if (this.$textHeight > h) this.$textHeight = h
      }
    }
  ...
  private renderWithBitmapFont(updateBounds: boolean): void {
    ...
    this.$btContainer.children.forEach((c, i) => {
      this.$bitmapPool.push(this.$btContainer.getChildAt(i) as Bitmap)
    }, this)

    this.$btContainer.removeAllChildren()

    if (!this.$lines) this.$lines = []
    else LineInfo.recycleMany(this.$lines)
    ...
    let lineWidth: number = 0,
      lineHeight: number = 0,
      lineTextHeight: number = 0
    let glyphWidth: number = 0,
      glyphHeight: number = 0
    let wordChars: number = 0,
      wordStart: number = 0,
      wordEnd: number = 0
    ...
    for (let offset: number = 0; offset < textLength; ++offset) {
      ...
      if (ch == '\n') {
        ...
        if (lineTextHeight == 0) {
          if (lastLineHeight == 0) lastLineHeight = Math.ceil(this.fontSize * fontScale)
      ...
      if (lineWidth != 0) lineWidth += letterSpacing
    ...
    if (this.$textWidth > 0) this.$textWidth += GTextField.GUTTER_X * 2
    ...
    let w: number,
      h: number = 0
    if (this.$widthAutoSize) {
      if (this.$textWidth == 0) w = 0
      else w = this.$textWidth
    } else w = this.width

    if (this.$heightAutoSize) {
      if (this.$textHeight == 0) h = 0
      else h = this.$textHeight
    } else h = this.height
    ...
    this.$lines.forEach(line => {
      ...
      if (this.align == AlignType.Center) lineIndent = (rectWidth - line.width) / 2
      else if (this.align == AlignType.Right) lineIndent = rectWidth - line.width
      else lineIndent = 0
      ...
      for (let j: number = 0; j < textLength; j++) {
        ...
        if (glyph != null) {
          ...
          let bm: Bitmap
          if (this.$bitmapPool.length) bm = this.$bitmapPool.pop()
          else bm = new Bitmap()
          bm.x = charX + lineIndent + Math.ceil(glyph.offsetX * fontScale)
          bm.y = line.y + charIndent + Math.ceil(glyph.offsetY * fontScale)
          bm.texture = (<createjs.SpriteSheetFrame>glyph.texture).image
          bm.textureRect = (<createjs.SpriteSheetFrame>glyph.texture).rect
  ...
  protected handleSizeChanged(): void {
    if (this.$updatingSize) return

    if (this.$bitmapFont != null) {
      if (!this.$widthAutoSize) this.render()
    } else {
      if (this.$inProgressBuilding) {
        this.$textField.width = this.width
        this.$textField.height = this.height
      } else {
        if (this.$autoSize == AutoSizeType.Shrink) this.shrinkTextField()
        else {
          if (!this.$widthAutoSize) {
            if (!this.$heightAutoSize) {
              this.$textField.width = this.width
              this.$textField.height = this.height
            } else this.$textField.width = this.width
          }
        }
      }
    }
  ...
  protected layoutAlign(): void {
    ...
    if (this.$verticalAlign == VertAlignType.Top || th == 0) this.$offset.y = GTextField.GUTTER_Y
    else {
      let dh: number = Math.max(0, this.height - th)
      if (this.$verticalAlign == VertAlignType.Middle) this.$offset.y = dh * 0.5
      else if (this.$verticalAlign == VertAlignType.Bottom) this.$offset.y = dh
    }
  ...
  protected handleXYChanged(): void {
    ...
    if (this.$displayObject) this.updatePosition()
  }
  ...
  public setupBeforeAdd(xml: XmlNode): void {
    ...
    if (str) {
      ...
      this.$widthAutoSize =
        this.$autoSize == AutoSizeType.Both || this.$autoSize == AutoSizeType.Shrink
      this.$heightAutoSize =
        this.$autoSize == AutoSizeType.Both || this.$autoSize == AutoSizeType.Height
    }
    ...
    if (str) {
      ...
      if (str) this.stroke = parseInt(str)
      else this.stroke = 1
    }
  }
  ...
  public setupAfterAdd(xml: XmlNode): void {
    ...
    if (str != null && str.length > 0) this.text = str
    this.$sizeDirty = false
  }
}
