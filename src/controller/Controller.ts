...
export class Controller extends createjs.EventDispatcher {
  private $name: string
  ...
  public set selectedIndex(value: number) {
    if (this.$selectedIndex != value) {
      if (value > this.$pageIds.length - 1) throw new Error(`index out of range: ${value}`)

      this.$updating = true
      this.$previousIndex = this.$selectedIndex
      this.$selectedIndex = value
      this.$parent.applyController(this)
      let event = new createjs.Event(StateChangeEvent.CHANGED, true, false)
  ...
  public get selectedPage(): string {
    ...
    else return this.$pageNames[this.$selectedIndex]
  }
  ...
  public get previousPage(): string {
    if (this.$previousIndex == -1) return null
    else return this.$pageNames[this.$previousIndex]
  }
  ...
  public removePage(name: string): void {
    ...
    if (i != -1) {
      ...
      if (this.$selectedIndex >= this.$pageIds.length) this.selectedIndex = this.$selectedIndex - 1
  ...
  public get selectedPageId(): string {
    if (this.$selectedIndex == -1) return null
    else return this.$pageIds[this.$selectedIndex]
  }
  ...
  public set oppositePageId(val: string) {
    ...
    else if (this.$pageIds.length > 1) this.selectedIndex = 1
  }

  public get previousPageId(): string {
    if (this.$previousIndex == -1) return null
    else return this.$pageIds[this.$previousIndex]
  }
  ...
  public createAction(type: string): Action {
    switch (type) {
  ...
  public setup(xml: XmlNode): void {
    ...
    if (col.length > 0) {
      ...
      col.forEach(cxml => {
        let action: Action = this.createAction(cxml.attributes.type)
        action.setup(cxml)
    ...
    if (str) {
      ...
      str.split(',').forEach(str => {
        if (str && str.length) {
          ...
          if (e < this.$pageIds.length) pt.toPage = [this.$pageIds[e]]
          str = str.substring(0, k)
          if (str != '*') {
            e = parseInt(str)
            if (e < this.$pageIds.length) pt.fromPage = [this.$pageIds[e]]
          }
          pt.stopOnExit = true
    ...
    if (this.$parent && this.$pageIds.length > 0) this.$selectedIndex = 0
