...
import { GearType, RelationType } from './config/Definitions'
...
export class RelationItem {
  ...
  public set target(value: GObject) {
    if (this.$target != value) {
      if (this.$target) this.releaseRefTarget(this.$target)
      this.$target = value
      if (this.$target) this.addRefTarget(this.$target)
    }
  }
  ...
  private applyOnSizeChanged(info: RelationDef): void {
    ...
    switch (info.type) {
      ...
      case RelationType.Left_Center:
        ...
        if (info.percent) v = (v / this.$targetWidth) * this.$target.$rawWidth
      ...
      case RelationType.Top_Middle:
        ...
        if (info.percent) v = (v / this.$targetHeight) * this.$target.$rawHeight
      ...
      case RelationType.RightExt_Right:
        ...
        else v = this.$owner.width - (targetX + this.$targetWidth)
        if (this.$owner != this.$target.parent) v += this.$owner.x
        if (info.percent) v = (v / this.$targetWidth) * this.$target.$rawWidth
        if (this.$owner != this.$target.parent)
          this.$owner.width = targetX + this.$target.$rawWidth + v - this.$owner.x
        else this.$owner.width = targetX + this.$target.$rawWidth + v
        break
      case RelationType.TopExt_Top:
        break
      case RelationType.TopExt_Bottom:
      ...
      case RelationType.BottomExt_Bottom:
        ...
        else v = this.$owner.$rawHeight - (targetY + this.$targetHeight)
        if (this.$owner != this.$target.parent) v += this.$owner.y
        if (info.percent) v = (v / this.$targetHeight) * this.$target.$rawHeight
        if (this.$owner != this.$target.parent)
          this.$owner.height = targetY + this.$target.$rawHeight + v - this.$owner.y
        else this.$owner.height = targetY + this.$target.$rawHeight + v
        break
    }
  }
  ...
  private releaseRefTarget(target: GObject): void {
    target.off(DisplayObjectEvent.XY_CHANGED, this.$targetXYChanged)
    target.off(DisplayObjectEvent.SIZE_CHANGED, this.$targetSizeChanged)
    target.off(DisplayObjectEvent.SIZE_DELAY_CHANGE, this.$targetSizeWillChange)
  }

  private $targetXYChanged(evt: createjs.Event): void {
    if (
  ...
  private $targetSizeChanged(evt: createjs.Event): void {
    if (this.$owner.relations.$dealing != null) return
