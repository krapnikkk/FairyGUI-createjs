...
export class DOMEventManager extends createjs.EventDispatcher {
  ...
  public constructor() {
    ...
    //mouse wheel
    const toBind: string[] =
      'onwheel' in document || (<any>document)['documentMode'] >= 9
        ? ['wheel']
  ...
  private onMouseWheel(event: any): void {
    ...
    if (!this.lowestDelta || absDelta < this.lowestDelta) {
      ...
      if (orgEvent.type === 'mousewheel' && absDelta % 120 === 0) this.lowestDelta /= 40
    }
    ...
    let evt = new createjs.Event(DisplayObjectEvent.MOUSE_WHEEL, true, false)
