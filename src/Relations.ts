...
export class Relations {
  ...
  public addItems(target: GObject, sidePairs: string): void {
    ...
    for (let i = 0; i < 2; i++) {
      ...
      if (t == -1) throw new Error('Invalid relation type')
  ...
  public contains(target: GObject): boolean {
    let length: number = this.$items.length
    for (let i: number = 0; i < length; i++) {
      if (this.$items[i].target == target) return true
    }
    return false
  }
  ...
  public setup(xml: XmlNode): void {
    xml.children.forEach(cxml => {
      if (cxml.nodeName != 'relation') return
      ...
      if (this.$owner.parent) {
        if (targetId) target = this.$owner.parent.getChildById(targetId)
        else target = this.$owner.parent
      } else {
        //call from the component's constructor
        target = (this.$owner as GComponent).getChildById(targetId)
      }
      if (target) this.addItems(target, cxml.attributes.sidePair)
    }, this)
  }
}
