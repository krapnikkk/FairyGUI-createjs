...
import { InteractiveEvents, RelationType } from './config/Definitions'
...
import { IUISource } from './display/IUISource'
...
export class Window extends GComponent {
  ...
  public set contentPane(val: GComponent) {
    if (this.$contentPane != val) {
      if (this.$contentPane != null) this.removeChild(this.$contentPane)
  ...
  public set closeButton(value: GObject) {
    if (this.$closeButton != null) this.$closeButton.removeClick(this.closeEventHandler)
    this.$closeButton = value
    if (this.$closeButton != null) this.$closeButton.click(this.closeEventHandler, this)
  }
  ...
  public set dragArea(value: GObject) {
    if (this.$dragArea != value) {
      if (this.$dragArea != null) {
        ...
        this.$dragArea.off(DragEvent.START, this.$dragStart)
      }

      this.$dragArea = value
      if (this.$dragArea != null) {
        if (this.$dragArea instanceof GGraph) this.$dragArea.drawRect(0, '#000', '#000')
  ...
  public showOn(root: IGRoot): void {
  ...
  public hideImmediately(): void {
    Decls.GRoot.inst.hideWindowImmediately(this)
  }

  public centerOn(r: GObject, autoUpdate: boolean = false): void {
  ...
  public showModalWait(msg?: string, cmd: number = 0): void {
    if (cmd != 0) this.$requestingCmd = cmd
  ...
  protected layoutModalWaitPane(msg?: string): void {
    if (this.$contentArea != null) {
      let pt: createjs.Point = this.$frame.localToGlobal()
  ...
  private $uiLoadComplete(): void {
    ...
    for (let i: number = 0; i < cnt; i++) {
      if (!this.$uiSources[i].loaded) return
    }
  ...
  public dispose(): void {
    ...
    if (this.$dragArea) this.$dragArea.off(DragEvent.START, this.$dragStart)
  ...
  protected closeEventHandler(evt: createjs.Event): void {
    this.hide()
  }

  private $onShown(target: createjs.DisplayObject): void {
    if (!this.$inited) this.init()
    else this.doShowAnimation()
  }

  private $onHidden(target: createjs.DisplayObject): void {
    this.closeModalWait()
    this.onHide()
  }

  private $mouseDown(evt: createjs.Event): void {
    if (this.isShowing && this.bringToFrontOnClick) this.bringToFront()
  }

  private $dragStart(evt: createjs.Event): void {
