...
import { VertAlignType } from '../config/Definitions'
...
export class InputElement extends createjs.EventDispatcher {
  ...
  private colorValue: string = '0xffffff'
  ...
  $show(): void {
    if (!this.htmlInput.isCurrentInput(this)) {
      ...
      for (let key in this.$attrsCache) this.inputElement.setAttribute(key, this.$attrsCache[key])
      this.inputDiv = this.htmlInput.$wrapper
    } else this.inputElement.onblur = null
  ...
  public set text(value: string) {
    ...
    if (this.inputElement) this.inputElement.value = this.textValue
  }

  public setColor(value: string): void {
    this.colorValue = value
    if (this.inputElement) this.setElementStyle('color', StringUtil.HEX2RGB(this.colorValue))
  }
  ...
  $removeFromStage(): void {
    if (this.inputElement) this.htmlInput.disconnect(this)
  }
  ...
  public resetInput(): void {
    if (this.inputElement) {
      ...
      if (textfield.multipleLine) this.setAreaHeight()
      else {
        ...
        if (textfield.height < textfield.fontSize) {
          ...
          this.setElementStyle(
            'padding',
            '0px 0px ' + textfield.fontSize * 0.5 * this.$scaleX + 'px 0px'
          )
        } else {
          this.setElementStyle('height', textfield.fontSize * this.$scaleY + 'px')
          ...
          let top = rap * vao
          let bottom = rap - top,
            fsy = textfield.fontSize * 0.5 * this.$scaleY
          if (bottom < fsy) bottom = fsy
          this.setElementStyle('padding', top + 'px 0px ' + bottom + 'px 0px')
        }
      }
