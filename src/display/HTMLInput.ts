...
export class HTMLInput {
  ...
  public static get inst(): HTMLInput {
    if (!HTMLInput.$instance) HTMLInput.$instance = new HTMLInput()
  ...
  public initialize(container: HTMLElement, view: HTMLCanvasElement): void {
    ...
    if (!this.$delegateDiv) {
      ...
      Decls.GRoot.inst.on(InteractiveEvents.Click, this.canvasClickHandler, this)
  ...
  private initInputElement(multiline: boolean): void {
    ...
    inputElement.oninput = e => {
      if (this.$input) this.$input.onInputHandler()
    }
  }
  ...
  public disconnect(ele: InputElement): void {
    if (this.$input == null || this.$input == ele) {
      ...
      if (this.$curEle) this.$curEle.blur()
    }
  }
  ...
  public clearInputElement(): void {
    if (this.$curEle) {
      ...
      let el2
      if (this.$singleLine == this.$curEle) el2 = this.$multiLine
      else el2 = this.$singleLine
