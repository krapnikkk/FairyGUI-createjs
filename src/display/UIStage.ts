...
class DefaultBoudingRectCalculator implements IBoundingRectCalculator {
  public getRect(
    ...
    if (!p)
      ...
      throw new Error(
        "Your view of PIXI are still in memory but not appended to DOM yet? it's necessary that there is a parent element to wrap your view up."
      )
    ...
    if (!rect || rect.width <= 0 || rect.height <= 0) {
      console.warn(
        'It seems that you did not set a explicit size for the parent element of your view, now fall back to window size instead.'
      )
    ...
    if (ret.width <= 0 || ret.height <= 0) {
      console.warn(
        'fetch container size to initialize PIXI in all ways have failed, now use default size (fallbackWidth / fallbackHeight) specified in the options instead.'
      )
...
export class UIStage extends createjs.EventDispatcher {
  protected $appContext: createjs.Stage
  ...
  protected $canvasMatrix: createjs.Matrix2D = new createjs.Matrix2D()
  ...
  public constructor(app: createjs.Stage, stageOptions?: UIStageOptions) {
    super()
    ...
    if (stageOptions instanceof DefaultUIStageOptions) opt = stageOptions
    else {
    ...
    ;(this.$appContext.canvas as HTMLCanvasElement).style.position = 'absolute'
    let container = (this.$appContext.canvas as HTMLCanvasElement).parentElement
    ...
    else containerPosition = style.position
    if (containerPosition == '' || containerPosition == 'static') {
      containerPosition = 'relative'
      container.style.position = containerPosition
    }

    HTMLInput.inst.initialize(container, this.$appContext.canvas as HTMLCanvasElement)
  ...
  public get nativeStage(): createjs.Stage {
    return this.$appContext
  }
  ...
  protected calculateStageSize(
    scaleMode: string,
    screenWidth: number,
    screenHeight: number,
    contentWidth: number,
    contentHeight: number
  ): { stageWidth: number; stageHeight: number; displayWidth: number; displayHeight: number } {
  ...
  updateScreenSize(): void {
    if (HTMLInput.isTyping) return //todo

    let canvas = this.$appContext.canvas as HTMLCanvasElement
    let canvasStyle: any = canvas.style
    // todo
    let rect = this.$sizeCalcer.getRect(
      canvas,
    ...
    if (orientation != StageOrientation.AUTO) {
      shouldRotate =
        (orientation != StageOrientation.PORTRAIT && rect.height > rect.width) ||
    ...
    let stageSize = this.calculateStageSize(
    ...
    if (canvas.width !== stageWidth) canvas.width = stageWidth
  ...
  public dispose(): void {
    ...
    while (i-- >= 0) {
      if (UIStageInst[i] === this) UIStageInst.splice(i, 1)
    }
  }
}
