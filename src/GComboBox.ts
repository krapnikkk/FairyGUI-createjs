...
import { GearType, InteractiveEvents, PopupDirection, RelationType } from './config/Definitions'
...
import { isColorableTitle } from './interface/IColorableTitle'
...
export class GComboBox extends GComponent {
  ...
  public get icon(): string {
    if (this.$iconObject) return this.$iconObject.icon
    else return null
  }

  public set icon(value: string) {
    if (this.$iconObject) this.$iconObject.icon = value
  ...
  public set items(value: string[]) {
    ...
    if (this.$items.length > 0) {
      ...
      this.text = ''
      if (this.$icons != null) this.icon = null
      this.$selectedIndex = -1
    }
    this.$itemsUpdated = true
  }
  ...
  public set selectedIndex(val: number) {
    if (this.$selectedIndex == val) return
  ...
  protected constructFromXML(xml: XmlNode): void {
    ...
    if (str) {
      ...
      if (!this.$dropdown)
        throw new Error(
          "the 'dropdown' is not specified, it must be a component definied in the package pool"
        )
      ...
      if (this.$list == null)
        throw new Error(
          `${this.resourceURL}: the dropdown component must have a GList child and named 'list'.`
        )
  ...
  public setupAfterAdd(xml: XmlNode): void {
    ...
    if (xml) {
      ...
      if (str) this.$visibleItemCount = parseInt(str)

      let col: XmlNode[] = xml.children
      if (col) {
        col.forEach((x: XmlNode, i: number) => {
          if (x.nodeName == 'item') {
            this.$items.push(x.attributes.title)
            this.$values.push(x.attributes.value)
            str = x.attributes.icon
            if (str) {
              if (!this.$icons) this.$icons = new Array<string>(length)
              this.$icons[i] = str
            }
          }
        })
      ...
      if (str) {
        if (str == 'up') this.$popupDir = PopupDirection.Up
        else if (str == 'auto') this.$popupDir = PopupDirection.Auto
      }
    }
  }
  ...
  private $popupWinClosed(evt: createjs.Event): void {
    if (this.$over) this.setState(GButton.OVER)
  ...
  private delayedClickItem(index: number): void {
    ...
    if (this.$selectedIndex >= 0) this.text = this.$items[this.$selectedIndex]
    else this.text = ''
  ...
  private $rollover(evt: createjs.Event): void {
    this.$over = true
    if (this.$down || (this.$dropdown && this.$dropdown.parent)) return
